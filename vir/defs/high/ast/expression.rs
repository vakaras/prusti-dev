...
pub enum EvalInContextKind {
    ...
    Predicate,
    /// The standard `eval_in_quantified(acc(...), ...)` context.
    QuantifiedPredicate,
...
pub struct EvalIn {
    /// Either a predicate access predicate if evaluated in a current state or a
    /// predicate access predicate wrapped in old if evaluated in an old state.
